...
# ══════════════════════════════════════════
# TF-IDF KEYWORD FUNCTIONS
# ══════════════════════════════════════════
def compute_tfidf_keywords(titles_series, top_n=25, ngram_max=2):
    """
    Compute TF-IDF scores for keywords across all video titles.
    TF-IDF = Term Frequency × Inverse Document Frequency
    - Rewards words that appear often in FEW titles (truly trending)
    - Penalizes words that appear in ALL titles (generic stop words)
    Returns DataFrame: keyword, tfidf_score, doc_freq, ngram
    """
    titles = titles_series.dropna().astype(str).tolist()
    if len(titles) < 2:
        return pd.DataFrame(columns=["keyword","tfidf_score","doc_freq","ngram"])

    try:
        vectorizer = TfidfVectorizer(
            max_features     = 200,
            stop_words       = "english",
            ngram_range      = (1, ngram_max),   # unigrams + bigrams
            min_df           = 2,                 # must appear in ≥2 docs
            max_df           = 0.85,              # ignore if in >85% of docs
            token_pattern    = r"(?u)\b[a-zA-Z][a-zA-Z0-9]{2,}\b",
            sublinear_tf     = True,              # log(1+TF) to reduce dominance of very frequent terms
        )
        ...
        # Document frequency (how many titles contain this term)
        ...
        df_kw = pd.DataFrame({
            ...
            "tfidf_score" : scores,
            "doc_freq"    : doc_freqs,
            "ngram"       : ["bigram" if " " in f else "unigram" for f in feature_names],
        })

        return df_kw.sort_values("tfidf_score", ascending=False).head(top_n).reset_index(drop=True)

    except Exception:
        # Fallback to simple count if TF-IDF fails (too few docs)
        sw = {"the","a","an","in","on","of","to","and","is","for","with","at","by","from",
              "this","that","are","was","it","be","or","but","not","have","has","will","can",
              "i","you","we","my","its","new","how","what","why","video","official","full"}
        words = []
        for t in titles_series.dropna():
            words += [w.lower() for w in re.findall(r'\b[a-zA-Z]{3,}\b', str(t))
                      if w.lower() not in sw]
        counts = Counter(words).most_common(top_n)
        return pd.DataFrame(counts, columns=["keyword","tfidf_score"]).assign(
            doc_freq=lambda x: x["tfidf_score"], ngram="unigram")

def plot_tfidf_bar(df_kw, title_suffix="", height=340):
    """Render a horizontal bar chart of TF-IDF keyword scores."""
    if len(df_kw) == 0:
        return None

    df_plot = df_kw.sort_values("tfidf_score", ascending=True).tail(20)

    # Color by ngram type
    color_map = {"unigram": "#38bdf8", "bigram": "#818cf8"}
    colors    = [color_map.get(n, "#38bdf8") for n in df_plot.get("ngram", ["unigram"]*len(df_plot))]

    fig = go.Figure(go.Bar(
        x           = df_plot["tfidf_score"],
        y           = df_plot["keyword"],
        orientation = "h",
        marker      = dict(color=colors, opacity=0.85,
                           line=dict(color="#0d1117", width=0.5)),
        text        = df_plot["tfidf_score"].apply(lambda x: f"{x:.3f}"),
        textposition= "outside",
        textfont    = dict(size=10, color="#adbac7"),
        hovertemplate = (
            "<b>%{y}</b><br>"
            "TF-IDF Score: %{x:.4f}<br>"
            "Doc Freq: %{customdata}<extra></extra>"
        ),
        customdata  = df_plot["doc_freq"].values if "doc_freq" in df_plot.columns else None,
    ))
    ...
    fig.update_layout(
        ...
        height        = height,
        margin        = dict(l=8, r=60, t=32, b=8),
        showlegend    = False,
        xaxis = dict(title="TF-IDF Score", gridcolor="#21262d",
    ...
    fig.add_annotation(
        ...
        text="<span style='color:#38bdf8'>■</span> Unigram  "
             "<span style='color:#818cf8'>■</span> Bigram",
        showarrow=False, font=dict(size=10, color="#8b949e"),
        align="right",
    )
    return fig

def plot_tfidf_wordcloud(df_kw, bg="#161b22"):
    """Render a word cloud weighted by TF-IDF score."""
    if len(df_kw) == 0:
        return None
    freq_dict = dict(zip(df_kw["keyword"], df_kw["tfidf_score"]))
    wc = WordCloud(
        width            = 700,
        height           = 260,
        background_color = bg,
...
def compute_forecast(df_live, horizon_h=24):
    ...
    for cat, grp in df_live.groupby("category"):
        ...
        if len(ts) < 2:
            ...
            results.append({"category":cat,"current_avg":round(current),"forecast_avg":round(current),"growth_pct":0.0})
            continue
...
NGRAM_MAP = {
    "Unigrams only (1-word)":        (1, 1),
    "Unigrams + Bigrams (1-2 word)": (1, 2),
    "Bigrams only (2-word)":         (2, 2),
}
...
if st.session_state.page == "live":
    ...
    df = raw_live.copy()
    if sel_cats != "All":      df = df[df["category"] == sel_cats]
    if sel_countries != "All": df = df[df["country"]  == sel_countries]
    if sel_sent != "All":      df = df[df["sentiment"]== sel_sent]
    ...
    with st.container():
        ...
        top_country["short"] = top_country["title"].str[:40]+"..."
        top_country = top_country.sort_values("views", ascending=True)
        fig_tc = px.bar(top_country, x="views", y="country", orientation="h",
            color="category", text="short", color_discrete_sequence=COLORS)
        fig_tc.update_traces(textposition="inside", textfont_size=11)
        fig_tc.update_layout(
            plot_bgcolor="#161b22", paper_bgcolor="#161b22",
            font=dict(color="#c9d1d9",size=12), height=360,
        ...
        st.markdown('<div class="chart-caption">Top trending video in each country — colored by category</div></div>', unsafe_allow_html=True)
        ...
        with r2c1:
            ...
            cat_agg = df.groupby("category").agg(
                ...
                avg_comments=("comments","mean"),total_videos=("video_id","count"),
                avg_eng=("engagement","mean")).reset_index()
            metric_map = {"Views":("avg_views","Avg Views"),"Likes":("avg_likes","Avg Likes"),
                          "Comments":("avg_comments","Avg Comments"),"Videos":("total_videos","Total Videos")}
            ...
            fig_cat.update_layout(**dark(340))
        ...
        with r2c2:
            ...
            fig_scat.update_layout(**dark(340,showlegend=True,
                legend=dict(bgcolor="#161b22",font=dict(color="#c9d1d9",size=10),
                           orientation="h",yanchor="bottom",y=1.02)))
        ...
        with r_top_c1:
            ...
            st.markdown(f'<div class="chart-wrap"><div class="chart-title">🏆 Top 10 Videos by {vid_metric}</div>', unsafe_allow_html=True)
            vid_col_map = {"Views":("views","sum","Total Views"),"Likes":("likes","sum","Total Likes"),
                           "Comments":("comments","sum","Total Comments"),"Videos":("video_id","count","Video Count")}
            ...
            st.markdown(f'<div class="chart-caption">Top 10 videos by {vid_metric.lower()}</div></div>', unsafe_allow_html=True)
        ...
        with r_top_c2:
            ...
            st.markdown(f'<div class="chart-wrap"><div class="chart-title">📡 Top 10 Channels by {ch_metric}</div>', unsafe_allow_html=True)
            ch_col_map = {"Views":("views","sum","Total Views"),"Likes":("likes","sum","Total Likes"),
                          "Comments":("comments","sum","Total Comments"),"Videos":("video_id","count","Video Count")}
            ...
            st.markdown(f'<div class="chart-caption">Top 10 channels by {ch_metric.lower()}</div></div>', unsafe_allow_html=True)
        ...
        with r4c2:
            st.markdown('<div class="chart-wrap"><div class="chart-title">🎭 Sentiment Trend by Category</div>', unsafe_allow_html=True)
        ...
        # ngram toggle
        tfidf_c1, tfidf_c2, tfidf_c3 = st.columns([2,2,2])
        with tfidf_c1:
            ngram_choice = st.selectbox("N-gram Range",
                ["Unigrams only (1-word)","Unigrams + Bigrams (1-2 word)","Bigrams only (2-word)"],
                index=1, key="live_ngram")
        with tfidf_c2:
            tfidf_view = st.selectbox("Visualisation",
                ["Bar Chart (ranked)","Word Cloud","Both"],
                index=2, key="live_tfidf_view")
        with tfidf_c3:
            tfidf_cat_filter = st.selectbox("Filter by Category",
                ["All Categories"] + list(all_cats), key="live_tfidf_cat")
      
        
        ng_min, ng_max = NGRAM_MAP[ngram_choice]

        tfidf_titles = df["title"] if tfidf_cat_filter == "All Categories" \
                       else df[df["category"]==tfidf_cat_filter]["title"]

        df_kw_live = compute_tfidf_keywords(tfidf_titles, top_n=25, ngram_max=ng_max)
        # Force min ngram
        if ng_min == 2:
            df_kw_live = df_kw_live[df_kw_live["ngram"] == "bigram"]

        tc1, tc2 = st.columns(2) if tfidf_view == "Both" else (st.columns(1)[0], None)

        if tfidf_view in ["Bar Chart (ranked)","Both"]:
            with (tc1 if tfidf_view=="Both" else st.container()):
                fig_tfidf = plot_tfidf_bar(df_kw_live, height=380)
                if fig_tfidf:
                    st.plotly_chart(fig_tfidf, use_container_width=True)
                else:
                    st.info("Not enough titles to compute TF-IDF (need 2+ videos)")

        if tfidf_view in ["Word Cloud","Both"] and tc2 is not None:
            with tc2:
                fig_wc = plot_tfidf_wordcloud(df_kw_live)
                if fig_wc:
                    st.markdown('<div style="margin-top:8px;"></div>', unsafe_allow_html=True)
                    st.pyplot(fig_wc)
                    if len(df_kw_live) > 0:
                        top_kw = df_kw_live.iloc[0]
                        st.markdown(f"""
                        <div style="background:#21262d;border-radius:8px;padding:10px 14px;margin-top:8px;">
                          <span style="color:#adbac7;font-size:11px;">🏅 Top keyword: </span>
                          <span style="color:#58a6ff;font-weight:700;font-size:13px;">"{top_kw['keyword']}"</span>
                          <span style="color:#adbac7;font-size:11px;"> · TF-IDF score: </span>
                          <span style="color:#3fb950;font-weight:700;">{top_kw['tfidf_score']:.4f}</span>
                        </div>
                        """, unsafe_allow_html=True)
        elif tfidf_view == "Word Cloud":
            fig_wc = plot_tfidf_wordcloud(df_kw_live)
            if fig_wc:
                st.pyplot(fig_wc)
        ...
        # TF-IDF table
        if len(df_kw_live) > 0:
            with st.expander("📋 Full TF-IDF Keyword Table"):
                show_df = df_kw_live.copy()
                show_df["tfidf_score"] = show_df["tfidf_score"].round(4)
                show_df["rank"]        = range(1, len(show_df)+1)
                show_df = show_df[["rank","keyword","tfidf_score","doc_freq","ngram"]]
                show_df.columns = ["Rank","Keyword","TF-IDF Score","Doc Frequency","Type"]
                st.dataframe(show_df, use_container_width=True, hide_index=True)

        st.markdown('<div class="chart-caption">Blue = single keyword (unigram) · Purple = 2-word phrase (bigram) · Higher TF-IDF = more distinctive to this dataset</div></div>', unsafe_allow_html=True)
        ...
        if len(fcast_df) > 0:
            ...
            st.markdown(f"""
            <div class="kpi-row" style="padding-top:16px;padding-bottom:12px;">
              <div class="kpi-card">
        ...
        with r5c1:
            ...
            if len(fcast_df) > 0:
                ...
                fig_fc.update_layout(**dark(320,showlegend=True,barmode="overlay",
                    legend=dict(bgcolor="#161b22",font=dict(color="#c9d1d9",size=10),
                                orientation="h",yanchor="bottom",y=1.02)))
                st.plotly_chart(fig_fc, use_container_width=True)
            else:
                st.info("Need 2+ collection runs for forecast")
            st.markdown('<div class="chart-caption">Blue = current avg · Purple = predicted 24h via linear regression</div></div>', unsafe_allow_html=True)

        with r5c2:
            st.markdown('<div class="chart-wrap"><div class="chart-title">📊 Forecast Growth % — Next 24h</div>', unsafe_allow_html=True)
            if len(fcast_df) > 0:
                ...
                fig_gr.update_layout(**dark(320,
                    xaxis=dict(title="Growth %",gridcolor="#21262d",linecolor="#444d56",zeroline=False),
                    margin=dict(l=8,r=70,t=32,b=8)))
                st.plotly_chart(fig_gr, use_container_width=True)
            else:
                st.info("Need 2+ collection runs for forecast")
            st.markdown('<div class="chart-caption">Green = predicted growth · Red = predicted decline</div></div>', unsafe_allow_html=True)
        ...
        # Prescriptive keyword insight from TF-IDF
        top_kw_str = ""
        if len(df_kw_live) > 0:
            top_kws   = df_kw_live.head(3)["keyword"].tolist()
            top_kw_str = ", ".join([f'"{k}"' for k in top_kws])
        ...
        ac1.markdown(f"""<div class="ins-box" style="border-left:4px solid #3fb950;margin-bottom:12px;">
            ...
            Avg views: <b>{best_views_val:,}</b><br>Source: Gold Layer · MongoDB</div>
        </div>""", unsafe_allow_html=True)
        ac2.markdown(f"""<div class="ins-box" style="border-left:4px solid #38bdf8;margin-bottom:12px;">
            ...
            TF-IDF trending: {top_kw_str}<br>Source: Predictive · 24h forecast</div>
        </div>""", unsafe_allow_html=True)
        ac3.markdown(f"""<div class="ins-box" style="border-left:4px solid #f85149;margin-bottom:12px;">
        ...
        ac4.markdown(f"""<div class="ins-box" style="border-left:4px solid #818cf8;margin-bottom:12px;">
            ...
            Target for sponsorships.<br>Source: Gold Layer · Country</div>
        </div>""", unsafe_allow_html=True)
        ac5.markdown(f"""<div class="ins-box" style="border-left:4px solid #f472b6;margin-bottom:12px;">
            ...
            <div class="ins-text">Views↔Likes correlation: <b>{corr_val}</b><br>
            Source: Gold Layer · Sentiment</div>
        </div>""", unsafe_allow_html=True)
        ac6.markdown(f"""<div class="ins-box" style="border-left:4px solid #fbbf24;margin-bottom:12px;">
            ...
            <div class="ins-text">"{top_title}"<br>Source: Gold Layer · Trending</div>
        </div>""", unsafe_allow_html=True)

        st.markdown(f"""
        <div style="background:#161b22;border:1px solid #30363d;border-radius:10px;
                    padding:14px 20px;margin-top:8px;display:flex;gap:32px;flex-wrap:wrap;">
            <div><span style="color:#adbac7;font-size:12px;text-transform:uppercase;">Best Category</span>
                 <div style="color:#3fb950;font-weight:700;font-size:15px;">{best_cat}</div></div>
            <div><span style="color:#adbac7;font-size:12px;text-transform:uppercase;">Best Country</span>
                 <div style="color:#38bdf8;font-weight:700;font-size:15px;">{best_country}</div></div>
            <div><span style="color:#adbac7;font-size:12px;text-transform:uppercase;">Best Tone</span>
                 <div style="color:#f472b6;font-weight:700;font-size:15px;">{best_sent.title()}</div></div>
            <div><span style="color:#adbac7;font-size:12px;text-transform:uppercase;">Fastest Growing</span>
                 <div style="color:{fastest_color};font-weight:700;font-size:15px;">{fastest_cat} {fastest_arrow}{abs(fastest_pct):.1f}%</div></div>
            <div><span style="color:#adbac7;font-size:12px;text-transform:uppercase;">Views↔Likes Corr</span>
                 <div style="color:#fbbf24;font-weight:700;font-size:15px;">{corr_val}</div></div>
            <div><span style="color:#adbac7;font-size:12px;text-transform:uppercase;">Data Source</span>
...
# ══════════════════════════════════════════════════════════════
    ...
    with st.container():
        ...
        with r1c1:
            ...
            st.markdown('<div class="chart-caption">Which countries produce the most trending content?</div></div>', unsafe_allow_html=True)

        with r1c2:
            st.markdown('<div class="chart-wrap"><div class="chart-title">📊 Category Views vs Engagement</div>', unsafe_allow_html=True)
            fig_ce = px.scatter(cat_f, x="avg_views", y="avg_engagement", size="total_videos",
                color="category", color_discrete_sequence=COLORS,
            ...
            st.markdown('<div class="chart-caption">Which categories have both high views AND engagement?</div></div>', unsafe_allow_html=True)

        r2c1,r2c2,r2c3 = st.columns(3)
        with r2c1:
            st.markdown('<div class="chart-wrap"><div class="chart-title">📅 Engagement by Day of Week</div>', unsafe_allow_html=True)
            day_f2 = day_perf.copy()
            ...
            fig_day.add_trace(go.Scatter(x=day_f2["published_day_of_week"],y=day_f2["avg_engagement"],
                name="Engagement",line=dict(color="#f472b6",width=2),yaxis="y2",mode="lines+markers"))
            fig_day.update_layout(
                plot_bgcolor="#161b22",paper_bgcolor="#161b22",font=dict(color="#c9d1d9",size=11),
                height=260,margin=dict(l=8,r=8,t=24,b=8),
        ...
        with r2c2:
            ...
            fig_hr = px.bar(hour_perf.sort_values("publish_hour"),
                x="publish_hour",y="avg_views",color="avg_views",color_continuous_scale="Blues")
        ...
        with r3c1:
            ...
            fig_corr = px.imshow(corr,color_continuous_scale="RdBu_r",zmin=-1,zmax=1,text_auto=True,aspect="auto")
        ...
        with r4c1:
            ...
            fig_sc2 = px.scatter(scatter_f.sample(min(500,len(scatter_f)),random_state=42),
                ...
                color_discrete_sequence=COLORS,opacity=0.6,hover_data=["title"])
        ...
        with r4c2:
            ...
            fig_tl = px.bar(title_eng,x="title_length_bucket",y="avg_engagement",
                color="avg_engagement",color_continuous_scale="Blues")
        ...
        with r4c3:
            ...
            fig_lr = px.bar(liker_f.sort_values("avg_like_ratio",ascending=False),
                x="category",y="avg_like_ratio",color="avg_like_ratio",color_continuous_scale="Greens")
            fig_lr.update_layout(**dark(260,xaxis=dict(tickangle=-30)))
        ...
        <div class="chart-wrap">
          <div class="chart-title">
            🔠 TF-IDF Keyword Intelligence — Historical Titles
          ...
          <div style="font-size:11px;color:#8b949e;margin-bottom:12px;">
            TF-IDF analysis on <b style="color:#58a6ff;">historical trending titles</b> — reveals which keywords have been
            consistently distinctive across the dataset. Compare with live TF-IDF to spot emerging vs established trends.
          </div>
        """, unsafe_allow_html=True)

        h_tfidf_c1, h_tfidf_c2, h_tfidf_c3 = st.columns([2,2,2])
        with h_tfidf_c1:
            h_ngram_choice = st.selectbox("N-gram Range",
                ["Unigrams only (1-word)","Unigrams + Bigrams (1-2 word)","Bigrams only (2-word)"],
                index=1, key="hist_ngram")
        with h_tfidf_c2:
            h_tfidf_view = st.selectbox("Visualisation",
                ["Bar Chart (ranked)","Word Cloud","Both"],
                index=2, key="hist_tfidf_view")
        with h_tfidf_c3:
            h_tfidf_cat = st.selectbox("Filter by Category",
                ["All Categories"]+list(all_cats_h), key="hist_tfidf_cat")

        h_ng_min, h_ng_max = NGRAM_MAP[h_ngram_choice]
        h_tfidf_titles = scatter["title"] if h_tfidf_cat=="All Categories" \
                         else scatter[scatter["category"]==h_tfidf_cat]["title"]
        df_kw_hist = compute_tfidf_keywords(h_tfidf_titles, top_n=25, ngram_max=h_ng_max)
        if h_ng_min == 2:
            df_kw_hist = df_kw_hist[df_kw_hist["ngram"]=="bigram"]

        hc1, hc2 = st.columns(2) if h_tfidf_view=="Both" else (st.columns(1)[0], None)

        if h_tfidf_view in ["Bar Chart (ranked)","Both"]:
            with (hc1 if h_tfidf_view=="Both" else st.container()):
                fig_h_tfidf = plot_tfidf_bar(df_kw_hist, height=380)
                if fig_h_tfidf:
                    st.plotly_chart(fig_h_tfidf, use_container_width=True)
                else:
                    st.info("Not enough historical titles to compute TF-IDF")

        if h_tfidf_view in ["Word Cloud","Both"] and hc2 is not None:
            with hc2:
                fig_h_wc = plot_tfidf_wordcloud(df_kw_hist)
                if fig_h_wc:
                    st.markdown('<div style="margin-top:8px;"></div>', unsafe_allow_html=True)
                    st.pyplot(fig_h_wc)
                    if len(df_kw_hist) > 0:
                        top_h_kw = df_kw_hist.iloc[0]
                        st.markdown(f"""
                        <div style="background:#21262d;border-radius:8px;padding:10px 14px;margin-top:8px;">
                          <span style="color:#adbac7;font-size:11px;">🏅 Top historical keyword: </span>
                          <span style="color:#58a6ff;font-weight:700;font-size:13px;">"{top_h_kw['keyword']}"</span>
                          <span style="color:#adbac7;font-size:11px;"> · TF-IDF score: </span>
                          <span style="color:#3fb950;font-weight:700;">{top_h_kw['tfidf_score']:.4f}</span>
                        </div>
                        """, unsafe_allow_html=True)
        elif h_tfidf_view == "Word Cloud":
            fig_h_wc = plot_tfidf_wordcloud(df_kw_hist)
            if fig_h_wc:
                st.pyplot(fig_h_wc)
        ...
        if len(df_kw_hist) > 0:
            with st.expander("📋 Full TF-IDF Keyword Table"):
                show_h = df_kw_hist.copy()
                show_h["tfidf_score"] = show_h["tfidf_score"].round(4)
                show_h["rank"]        = range(1,len(show_h)+1)
                show_h = show_h[["rank","keyword","tfidf_score","doc_freq","ngram"]]
                show_h.columns = ["Rank","Keyword","TF-IDF Score","Doc Frequency","Type"]
                st.dataframe(show_h, use_container_width=True, hide_index=True)

        st.markdown('<div class="chart-caption">Blue = single keyword · Purple = 2-word phrase · Higher TF-IDF = more distinctive trending term</div></div>', unsafe_allow_html=True)
        ...
        with r5c1:
            ...
            cat_f2["simulated_growth_pct"] = ((cat_f2["avg_engagement"]-eng_mean)/eng_std*10).round(1)
            ...
            fig_hfc = go.Figure(go.Bar(
                x=cat_f2["simulated_growth_pct"],y=cat_f2["category"],orientation="h",
            ...
            fig_hfc.update_layout(**dark(340,
                xaxis=dict(title="Simulated Growth %",gridcolor="#21262d",linecolor="#444d56",zeroline=False),
                margin=dict(l=8,r=70,t=32,b=8)))
            st.plotly_chart(fig_hfc, use_container_width=True)
            st.markdown('<div class="chart-caption">Engagement-normalised forecast — categories above mean trend positive</div></div>', unsafe_allow_html=True)

        with r5c2:
            st.markdown('<div class="chart-wrap"><div class="chart-title">⏰ Best Publish Hour — Peak Window</div>', unsafe_allow_html=True)
            fig_hr2 = go.Figure()
            ...
            fig_hr2.update_layout(**dark(340,
                xaxis=dict(title="Hour (24h)",gridcolor="#21262d",linecolor="#444d56",dtick=2),
                yaxis=dict(title="Avg Views",gridcolor="#21262d",linecolor="#444d56")))
        ...
        with r5c3:
            st.markdown('<div class="chart-wrap"><div class="chart-title">🎭 Sentiment by Category</div>', unsafe_allow_html=True)
            fig_sc3 = px.bar(sent_cat_f,x="category",y="count",color="sentiment",barmode="stack",
                color_discrete_map={"positive":"#3fb950","neutral":"#8b949e","negative":"#f85149"})
        ...
        with r5c4:
            # Simple count-based word cloud kept as secondary comparison
            st.markdown('<div class="chart-wrap"><div class="chart-title">☁️ Frequency Word Cloud (Count-based)</div>', unsafe_allow_html=True)
            ...
            if kw_dict:
                ...
                ax2.axis("off")
                st.pyplot(fig_wc2)
            st.markdown('<div class="chart-caption">Simple frequency-based cloud — compare with TF-IDF above to see which keywords are truly distinctive vs just common</div></div>', unsafe_allow_html=True)
        ...
        for col,title,val,desc in items:
            col.markdown(f"""<div class="ins-box">
